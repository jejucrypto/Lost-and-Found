...
function displayCardSwiperDetails() {
  const cardWrapper = document.getElementById("card-wrapper");
  const swiperOverlay = document.getElementById('swiper-overlay');
  const fullDetails = document.getElementById('swiper-overlay-content');
  const cardUnderList = document.getElementById('cardUnderlist');

  let cardCount = 0;

  cardWrapper.innerHTML = ""; // Clear existing items
  fullDetails.innerHTML = "";
  cardUnderList.innerHTML = "";
  ...
  reports.forEach(report => {
    if (cardCount < 3) {
      let html = `  
                        <li class="card-item swiper-slide" data-item-name="${report.itemName}" data-report='${JSON.stringify(report)}'>
        <a href="#" class="card-link">
              <p class="item-lost">Item Lost:</p>
              <h1 class="item-lost-name">${report.itemName.charAt(0).toUpperCase() + report.itemName.slice(1)}</h1>
              <p class="status">Status: ${report.itemType.charAt(0).toUpperCase() + report.itemType.slice(1)}</p>
              <p class="date">Date: ${report.dateReported}</p>
              <p class="click-for-more">Click for more info . . . </p>
        </a>
  </li>
   `;
      cardUnderList.innerHTML += html;
      cardCount++;
    }
  ...
  cardContainers.forEach((container) => {
    container.addEventListener('click', () => {
      ...
      if (swiperOverlay) {
        ...
        messageBtn.forEach((clickedMesage) => {
